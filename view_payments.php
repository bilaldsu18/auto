...
<tr>
    ...
    <th>Payable</th>
    <th style='width:8%'>Receive</th>
    <th style='width:8%'>Balance</th>
    <th style='width:5%'>Lease</th>
    <th style='width:10%'>Install</th>
    <th>Payment</th>
</tr>
...
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!=""){
    ...
    $sql = "SELECT DISTINCT(transactionid) FROM  stock_sales WHERE stock_name LIKE '%$searchtxt%' OR customer_id LIKE '%$searchtxt%' OR reg_no LIKE '$searchtxt' OR eng_no LIKE '$searchtxt' OR chas_no LIKE '$searchtxt' OR transactionid  LIKE '%$searchtxt%' OR date LIKE '%$searchtxt%' ORDER BY date desc LIMIT $start, $limit";
}
...
while($row = mysql_fetch_array($result)){
    ...
    $dueDate = $line->due;
     ...
     $dueDate1 = substr($dueDate,5,2);
    $dueDate2 = substr($dueDate,8);
    $shufshuf = @date('d');

    $alert1 = @date('m', strtotime("-1 months"));
    $alert2 = @date('m', strtotime("-2 months"));
    $alert3 = @date('m', strtotime("-3 months"));
    $alert4 = @date('m', strtotime("-4 months"));
    $color = '';
    if($alert1 == $dueDate1){
        if($shufshuf >= $dueDate2){
            $color = 'lightgreen';
        }
    }
    if($alert2 == $dueDate1){
    ...
    <tr style='background-color: <?php echo $color; ?>'>
        ...
        <td width="100"><?php echo $line->amount; ?></td>
        <td width="100"><?php echo $line->dis_amount; ?></td>
        <td width="100"><?php echo $line->subtotal; ?></td>
        <td width="100"><?php echo $line->payment; ?></td>
        <td width="100"><?php echo $line->balance; ?></td>
        <td width="100"><?php echo $line->installment; ?></td>
        <td width="100"><?php echo $line->inst_amount; ?></td>
        <td>
            <a href="update_payment.php?sid=<?php echo $line->transactionid;?>&table=stock_entries&return=view_payments.php">Pay now</a>&nbsp;| &nbsp;
            <a href="report_payments.php?sid=<?php echo $line->transactionid;?>&table=stock_entries&return=view_payments.php">Detail</a>
        </td>
    </tr>
    <?php $i++; } ?>
<tr>

    <td align="center" colspan='10'><div style="margin-left:20px;"><?php echo $pagination; ?></div></td>
