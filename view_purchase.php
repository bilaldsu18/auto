...
//    $sql = "SELECT DISTINCT(stock_id) FROM stock_entries WHERE stock_name LIKE '%".$_POST['searchtxt']."%' OR stock_supplier_name LIKE '%".$_POST['searchtxt']."%' OR stock_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."%' OR type LIKE '%".$_POST['searchtxt']."%' and type='entry' ORDER BY date desc LIMIT $start, $limit";
    echo $sql = "SELECT DISTINCT(stock_id) FROM stock_entries WHERE billnumber = '$searchtxt' ORDER BY date desc LIMIT $start, $limit";
...
$sel = "SELECT * FROM stock_entries group by stock_id LIMIT $start, $limit ";
if($_POST['Search'] AND trim($_POST['searchtxt'])!=""){
    $searchtxt = $_POST['searchtxt'];
    $sel = "SELECT * FROM stock_entries WHERE stock_name LIKE '%$searchtxt%' OR stock_supplier_name LIKE '%$searchtxt%' OR chas_no LIKE '%$searchtxt%' OR eng_no LIKE '%$searchtxt%' OR reg_no LIKE '%$searchtxt%' OR billnumber LIKE '%$searchtxt%' OR stock_id LIKE '%$searchtxt%' OR date LIKE '%$searchtxt%' OR type LIKE '%$searchtxt%' and type='entry' group by stock_id LIMIT $start, $limit";
}
$sql = mysql_query($sel);
while($row = mysql_fetch_array($sql)){
    ...
    <tr>
        ...
        <td width="100"><?php echo $line->billnumber; ?></td>
        <td width="100"><?php echo $line->stock_supplier_name; ?></td>
        <td width="100"><?php echo $line->subtotal; ?></td>
        <td>
            <a href="update_purchase.php?sid=<?php echo $entryid;?>&table=stock_entries&return=view_purchase.php"	class="table-actions-button ic-table-edit">
            </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <a  href="javascript:confirmSubmit(<?php echo $line->id;?>,'stock_entries','view_purchase.php')" class="table-actions-button ic-table-delete">
            </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <input type="checkbox" value="<?php echo $line->id; ?>" name="checklist[]" id="check_box" />
        </td>
...
<tr>
    ...
    <td align="center" colspan='11'><div style="margin-left:20px;"><?php echo $pagination; ?></div></td>
