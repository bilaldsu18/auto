...
else
{
	...
	if(isset($_REQUEST['table']) && isset($_REQUEST['checklist']))
	{
           ...
           for($i=0;$i<count($data);$i++){
			 ...
			 $dlt_stock_name =$_POST['dlt_stock_name'];
			 $db->execute("DELETE FROM stock_avail WHERE name='$dlt_stock_name'"); 
...
if(isset($_REQUEST['dlt_stock_name'])){
				 $dlt_stock_name =$_REQUEST['dlt_stock_name'];
			 $db->execute("DELETE FROM stock_avail where name='$dlt_stock_name'"); 
}
?>