...
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
    ...
    $SQL = "SELECT DISTINCT(transactionid) FROM  stock_sales WHERE stock_name LIKE '%".$_POST['searchtxt']."%' OR customer_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."'";
...
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
    ...
    $query = "SELECT COUNT(transactionid) as num FROM stock_sales WHERE stock_name LIKE '%".$_POST['searchtxt']."%' OR customer_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."'";
...
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
    ...
    $sql= "SELECT * FROM stock_sales WHERE stock_name LIKE '%".$_POST['searchtxt']."%' OR customer_id LIKE '%".$_POST['searchtxt']."%' OR date LIKE '%".$_POST['searchtxt']."' ORDER BY date desc LIMIT $start, $limit";
...
<tr>
    ...
    <th>Edit / Delete / Select</th>
</tr>

<?php
$i=1; $no=$page-1; $no=$no*$limit;
$sel1 = "SELECT * FROM stock_sales GROUP BY transactionid LIMIT $start, $limit";
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!=""){
    $searchtxt = $_POST['searchtxt'];
    $sel1= "SELECT * FROM stock_sales WHERE stock_name LIKE '%$searchtxt%' OR customer_id LIKE '%$searchtxt%' OR reg_no = '$searchtxt' OR eng_no = '$searchtxt' OR chas_no = '$searchtxt' OR date LIKE '%$searchtxt' OR cust_cat = '$searchtxt' GROUP BY transactionid LIMIT $start, $limit";
}
$run = mysql_query($sel1);
while($row1 = mysql_fetch_array($run)){
    ...
    <tr>
        ...
        <td><?php echo $row1['chas_no']; ?></td>
        <td> <?php echo $row1['grand_total']; ?></td>
        <td><?php echo "<a href='sale_bill.php?transactionid=$transactionid' target='blank'>Bill</a>&nbsp;&nbsp;&nbsp;"; ?>
            <a href="update_sales.php?sid=<?php echo $row1['id'];?>&table=stock_sales&return=view_sales.php"	class="table-actions-button ic-table-edit">
            </a>&nbsp;&nbsp;
            <a  href="javascript:confirmSubmit(<?php echo $row1['id']?>,'stock_sales','view_sales.php')" class="table-actions-button ic-table-delete"></a>
        <input type="checkbox" value="<?php echo $row1['id']; ?>" name="checklist[]" id="check_box" />
        </td>
    </tr>
    <?php $i++; } ?>
<tr>
