            ...
            if(r){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        window.location = "view_reg_excise.php";
...
$SQL = "SELECT * FROM  registration_excise";
...
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
{
    $query = "SELECT COUNT(*) as num FROM  registration_excise WHERE name  LIKE '%".$_POST['searchtxt']."%' OR f_name LIKE '%".$_POST['searchtxt']."%' OR chas_no LIKE '%".$_POST['searchtxt']."%'";
}
...
if($lastpage > 1)
{
    ...
    //previous button
    if ($page > 1)
        $pagination.= "<a href=\"view_reg_excise.php?page=$prev&limit=$limit\" class=my_pagination >Previous</a>";
    else
...

    {
...

        {
            ...
            if ($counter == $page)
                ...
                $pagination.= "<a href=\"view_reg_excise.php?page=$counter&limit=$limit\" class=my_pagination>$counter</a>";
...

    {
...

        {
            ...
            $pagination.= "<a href=\"view_reg_excise.php?page=$lpm1&limit=$limit\" class=my_pagination>$lpm1</a>";

            $pagination.= "<a href=\"view_reg_excise.php?page=$lastpage&limit=$limit\" class=my_pagination>$lastpage</a>";
...

        {
            ...
            $pagination.= "<a href=\"view_reg_excise.php?page=1&limit=$limit\" class=my_pagination>1</a>";

            $pagination.= "<a href=\"view_reg_excise.php?page=2&limit=$limit\" class=my_pagination>2</a>";
    ...
    if ($page < $counter - 1)
        ...
        $pagination.= "<a href=\"view_reg_excise.php?page=$next&limit=$limit\" class=my_pagination>Next</a>";
...
<tr style="text-align: left">
    ...
    <th >S/O</th>
    <th >Care/Of</th>
    <th style='width:8%;'>Eng.No</th>
    <th style='width:8%;'>Chas.No</th>
...
$sql = "SELECT * FROM registration_excise LIMIT $start, $limit ";
if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!=""){
    $searchtxt = $_POST['searchtxt'];
    $sql= "SELECT * FROM  registration_excise WHERE name  LIKE '%$searchtxt%' OR f_name LIKE '%$searchtxt%' OR eng_no = '$searchtxt' OR status = '$searchtxt' OR reg_no = '$searchtxt' OR chas_no LIKE '%$searchtxt%'  LIMIT $start, $limit";
}
...
while($row = mysql_fetch_array($result))
{
    ...
    <tr>
        ...
        <td><?php echo $row['care_of']; ?></td>
        <td><?php echo $row['chas_no']; ?></td>
        <td><?php echo $row['eng_no']; ?></td>
        <td><?php echo $row['reg_no']; ?></td>
        <td><?php echo $row['file_rec']; ?></td>
        <td><?php echo $row['status']; ?></td>
        <td><?php echo $row['no_plateR']; ?></td>
        <td><?php echo $row['no_plateD']; ?></td>

        <td>
            <a href="update_reg_excise.php?sid=<?php echo $row['id'];?>&table=registration_excise&return=view_reg_excise.php"	class="table-actions-button ic-table-edit">
            </a> &nbsp;
            <a  href="javascript:confirmSubmit(<?php echo $row['id'];?>, 'registration_excise','view_reg_excise.php')" class="table-actions-button ic-table-delete"></a>
        </td>
